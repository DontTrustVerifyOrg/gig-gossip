...
ScheduleItem = namedtuple('ScheduleItem', 'event start_at repeat_every num_repeats')
...
class Scheduler:
    ...
    def schedule(self,job,on_return,start_at,repeat_every=None,num_repeats=None):
        self._event_list.append(ScheduleItem(ScheduleJob(job,on_return),start_at,repeat_every,num_repeats))

    def _schedule_event_generator(self):
        cur = -1
        while(True):
            oneshot = [m for m in self._event_list if m.repeat_every is None and m.start_at>cur]
            cyclic = [m for m in self._event_list if m.repeat_every is not None and (m.num_repeats is None or cur < m.start_at+m.repeat_every*(m.num_repeats-1))]
            ...
            if cyclic:
                ...
                nexts = np.maximum(np.floor((cur - np.array(sched)) / np.array(perio)).astype(int)+1, 0) * perio + sched
                mincyc = np.min(nexts)
