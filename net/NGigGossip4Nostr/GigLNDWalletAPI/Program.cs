...
app.MapGet("/getaccountinfo", async (string authToken) =>
{
    try
    {
        return new Result<AccountInfo>(await Singlethon.LNDWalletManager.ValidateAuthTokenAndGetAccount(authToken).GetAccountInfo());
    }
    catch (Exception ex)
    {
        TraceEx.TraceException(ex);
        return new Result<AccountInfo>(ex);
    }
})
.WithName("GetAccountInfo")
.WithSummary("Returns information about the account like currency and country of fiat")
.WithDescription("Returns information about the account, including details such as the currency and country of the fiat account.")
.WithOpenApi(g =>
{
