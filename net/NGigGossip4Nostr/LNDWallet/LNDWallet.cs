...
public class LNDAccountManager
{
    ...
    public Invoicesrpc.AddHoldInvoiceResp AddHodlInvoice(long satoshis, string memo, byte[] hash, ulong txfee, long expiry = 86400)
    {
        ...
        walletContext.Invoices.Add(new Invoice() { hash = hash.AsHex(), pubkey = account, txfee = txfee });
        walletContext.SaveChanges();
        return inv;
    }
    ...
    public Lnrpc.AddInvoiceResponse AddInvoice(long satoshis, string memo, ulong txfee)
    {
        ...
        walletContext.Invoices.Add(new Invoice() { hash = inv.RHash.ToArray().AsHex(), pubkey = account, txfee = txfee });
        walletContext.SaveChanges();
        return inv;
    }

    public AsyncServerStreamingCall<Lnrpc.Payment> SendPayment(string paymentRequest, int timeout, ulong txfee)
    {
    ...
    public long GetAccountBallance()
    {
        ...
        var myinvoiceHashes = new Dictionary<string, ulong>(from inv in walletContext.Invoices where inv.pubkey == account select new KeyValuePair<string, ulong>(inv.hash, inv.txfee));
        var mypaymentHashes = new Dictionary<string, ulong>(from pay in walletContext.Payments where pay.pubkey == account select new KeyValuePair<string, ulong>(pay.hash, pay.txfee));

        var paidinvoices = (from inv in LND.ListInvoices(conf, idx).Invoices
        ...
        return channelfunds - alreadypayedout + paidinvoices - mypayments;
    }
